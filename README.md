...
[Releases](https://github.com/yvesanana-sys/Sayaw/releases). It is not
code-signed yet, so SmartScreen warns on first run: **More info** →
**Run anyway**.

**Linux and macOS** — build from source; it takes about five minutes.

Full instructions, including the Linux build dependencies and what to do when
there is no sound, are in [`INSTALL.md`](INSTALL.md).
